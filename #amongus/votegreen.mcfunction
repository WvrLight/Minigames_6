tag @s add auVotedGreen
execute if score @s auvoting matches 1 run scoreboard players add @a[team=au_green] auvotedummy 1
execute if score @s auvoting matches 1 run tellraw @s ["",{"text":"✔ Voted for ","color":"white"},{"selector":"@a[team=au_green]"}]

tag @s add auVotedLime
execute if score @s auvoting matches 1 run scoreboard players add @a[team=au_lime] auvotedummy 1
execute if score @s auvoting matches 1 run tellraw @s ["",{"text":"✔ Voted for ","color":"white"},{"selector":"@a[team=au_lime]"}]

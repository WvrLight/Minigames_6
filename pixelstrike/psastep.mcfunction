...
#SOUNDS
#Walk
execute as @s[scores={psStep=17}] if score @s IAwalk matches 1.. at @s run playsound minecraft:block.stone.step master @a ~ ~ ~ 2 1
execute as @s[scores={psStep=11}] if score @s IAwalk matches 1.. run scoreboard players set @s psStep 18

#Sprint
execute as @s[scores={psStep=..11}] unless score @s psSneak matches 1 if score @s IAsprint matches 1.. at @s run playsound minecraft:block.stone.hit master @a ~ ~ ~ 2 1
...
#Jump
#End of jump
execute as @s[scores={psStep=19..}] if entity @s[gamemode=!spectator,nbt={OnGround:1b}] if score @s psStep matches 30.. at @s run playsound minecraft:block.stone.fall master @a ~ ~ ~ 2 0

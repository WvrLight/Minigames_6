...
execute at @e[name=ConquerTarget] run spreadplayers ~ ~ 3 3 false @e[type=armor_stand,name=Soldier]
...
#execute if entity @s[scores={summoncharge=3}] at @e[name=ConquerTarget] run tp @e[name=Soldier,tag=stp,limit=1] ^-2 ^ ^
#execute if entity @s[scores={summoncharge=3}] run tag @e[name=Soldier,tag=stp,limit=1] remove stp
execute as @e[name=Soldier] at @s run function datapack:pvp/soldierdmg
kill @e[name=ConquerTarget]

clear @s minecraft:carrot_on_a_stick{conquer:1} 1
item replace entity @s hotbar.7 minecraft:orange_stained_glass_pane{display:{Name:"{\"text\":\"Ability on Cooldown\",\"color\":\"red\",\"bold\":true}",Lore:["{\"text\":\"This ability is on cooldown\",\"color\":\"gray\",\"italic\":true}"]},HideFlags:1,Enchantments:[{id:"minecraft:protection",lvl:1s}]} 4
